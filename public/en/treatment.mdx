...
<Gutter>
  <Card>
    ...
    <Card.Content>
      A newly developing area of bottom surgery is in non-binary operations which attempt to provide both forms of genitalia for the subject. For AMABs there is "penile preserving vaginoplasty", which crafts a vagina canal without taking tissue from the penis. For AFABs there is "vagina preserving phalloplasty", which constructs a penis from the clitorus and a skin graft (usually from the leg). These particular surgeries are extremely experimental, but there are no a half dozen surgeons providing these gender affirming operations. The outlook for the future is good.

      An additional option for non-binary bottom surgery is genital nullification surgery, which completely removes the external genitalia, leaving only a urethral opening.
    </Card.Content>
  </Card>
</Gutter>
